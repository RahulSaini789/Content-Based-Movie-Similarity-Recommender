...
def same_language(idx1: int, idx2: int) -> bool:
    global df
    # guard against uninitialized DataFrame
    if df is None:
        return True  # fail open, not crash

    try:
        return (
            df.iloc[idx1]["original_language"]
            == df.iloc[idx2]["original_language"]
        )
...
def load_pickles():
    ...
    if "original_language" not in df.columns:
        raise RuntimeError("df.pkl must contain 'original_language' column")
